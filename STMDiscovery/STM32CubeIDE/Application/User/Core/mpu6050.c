...
uint8_t MPU6050_Init(I2C_HandleTypeDef *I2Cx, uint8_t Gyro_FS, uint8_t Acc_FS, uint8_t DLPF_CFG)
{
	...
	// Read Who am I
	HAL_I2C_Mem_Read(I2Cx, MPU6050_ADDR, WHO_AM_I_REG, 1, &MPU6050_rx, 1, 10);
	...
	if (MPU6050_rx == 0x68)
	{
		...
		HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, CONFIG_REG, 1, &MPU6050_tx, 1, 10);
		HAL_Delay(10);

		MPU6050_tx = Gyro_FS << 3;
		HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, GYRO_CONFIG_REG, 1, &MPU6050_tx, 1, 10);
		HAL_Delay(10);

		MPU6050_tx = Acc_FS << 3;
		HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, ACCEL_CONFIG_REG, 1, &MPU6050_tx, 1, 10);
		HAL_Delay(10);
...
void MPU6050_Bypass(I2C_HandleTypeDef *I2Cx)
{
	...
	HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, 0x6A, 1, &MPU6050_tx, 1, 10); //Master Disable
	HAL_Delay(1);

	MPU6050_tx = 0b00000010; //
	HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, 0x37, 1, &MPU6050_tx, 1, 10); //Bypass Enable
	HAL_Delay(1);
}
...
{
	MPU6050_tx = 0x00; //
	HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, 0x37, 1, &MPU6050_tx, 1, 10); //Disable Bypass
	HAL_Delay(1);

	MPU6050_tx = 0b00100010; //
	HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, 0x6A, 1, &MPU6050_tx, 1, 10); //Master Enable
	HAL_Delay(1);

	MPU6050_tx = 0b00001101; //
	HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, 0x24, 1, &MPU6050_tx, 1, 10); //Master Clock to 400kHz
	HAL_Delay(1);
...
void HMC5883L_Setup(I2C_HandleTypeDef *I2Cx)
{
	...
	HAL_I2C_Mem_Write(I2Cx, HMC5883L_ADDRESS << 1, 0x00, 1, &MPU6050_tx, 1, 10);
	HAL_Delay(1);

	MPU6050_tx = 0b00100000; //Fill Slave0 DO
	HAL_I2C_Mem_Write(I2Cx, HMC5883L_ADDRESS << 1, 0x01, 1, &MPU6050_tx, 1, 10);
	HAL_Delay(1);

	MPU6050_tx = 0x00;
	HAL_I2C_Mem_Write(I2Cx, HMC5883L_ADDRESS << 1, 0x02, 1, &MPU6050_tx, 1, 10); //Mode: Continuous
	HAL_Delay(1);
}
...
void MPU6050_Slave_Read(I2C_HandleTypeDef *I2Cx)
{
	...
	HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, 0x25, 1, &MPU6050_tx, 1, 10);
	HAL_Delay(1);

	MPU6050_tx = 0x03; //Slave REG for reading to take place
	HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, 0x26, 1, &MPU6050_tx, 1, 10);
	HAL_Delay(1);

	MPU6050_tx = 0x80 | 0x06; //Number of data bytes
	HAL_I2C_Mem_Write(I2Cx, MPU6050_ADDR, 0x27, 1, &MPU6050_tx, 1, 10);
	HAL_Delay(1);
}

uint8_t MPU6050_DataReady(I2C_HandleTypeDef *I2Cx)
{
	HAL_I2C_Mem_Read(I2Cx, MPU6050_ADDR, INT_STATUS_REG, 1, &MPU6050_rx, 1, 10);
	return MPU6050_rx;
}
